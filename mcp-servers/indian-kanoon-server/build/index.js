...
const isValidSearchParams = (args) => typeof args === "object" &&
    args !== null &&
...
const isValidDocumentParams = (args) => typeof args === "object" &&
    args !== null &&
    typeof args.docId === "string" &&
    (args.maxcites === undefined || typeof args.maxcites === "number") &&
    (args.maxcitedby === undefined || typeof args.maxcitedby === "number");
const isValidDocumentFragmentParams = (args) => typeof args === "object" && args !== null && typeof args.docId === "string" && typeof args.formInput === "string";
...
class IndianKanoonServer {
    ...
    setupToolHandlers() {
        this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
            tools: [
                {
                    name: "search_cases",
                    description: "Search for legal documents in the Indian Kanoon database. Supports various filters like date ranges, document types, courts, and more. Returns matching documents with titles, headlines, and metadata.",
                    inputSchema: {
                        type: "object",
                        properties: {
                            formInput: {
                                type: "string",
                                description: 'Search query. Can include phrases in quotes, and operators ANDD, ORR, NOTT (case-sensitive, need spaces). Example: "freedom of speech" ANDD NOTT censorship',
                            },
                            pagenum: {
                                type: "number",
                ...
                {
                    name: "get_document",
                    description: "Retrieve the full text of a legal document by its ID. Returns the complete document content, citations, and metadata.",
                    inputSchema: {
                        type: "object",
                        properties: {
                            docId: {
                ...
                },
                {
                    ...
                    description: "Get specific fragments of a document that match a search query. Useful for finding relevant sections within a large document.",
                    inputSchema: {
                        type: "object",
                        properties: {
                            docId: {
                ...
                },
                {
                    ...
                    description: "Retrieve metadata about a document including citations, title, court, date, and other bibliographic information.",
                    inputSchema: {
                        type: "object",
                        properties: {
                            docId: {
    ...
    async handleSearchCases(args) {
        ...
        try {
            ...
            if (args.cite)
                params.cite = args.cite;
            if (args.author)
                params.author = args.author;
            if (args.bench)
                params.bench = args.bench;
            if (args.maxcites)
                params.maxcites = Math.min(args.maxcites, 50);
            if (args.maxpages)
                params.maxpages = Math.min(args.maxpages, 1000);
    ...
    async handleGetDocument(args) {
        ...
        try {
            ...
            if (args.maxcites)
                params.maxcites = Math.min(args.maxcites, 50);
            if (args.maxcitedby)
                params.maxcitedby = Math.min(args.maxcitedby, 50);
